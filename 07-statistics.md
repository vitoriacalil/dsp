...
fraternal = 1/125
```
We know that Elvis twin was male
let's set probabilities of twin being the same sex
```Python
result = (same_identical * identical)/((same_identical * identical) + (same_fraternal * fraternal))
```

There is 45.45% chance that Elvis twin was identical.
